...
function ui(){
  ...
  $("valEnergy").textContent=Math.max(0,dE_info.available).toFixed(4)+" J";
  if($("energyDetailLine")) $("energyDetailLine").textContent=
  ...
  if($("energyStatusDetail")) $("energyStatusDetail").textContent=detail+"  |  |ω| = "+absOmega.toFixed(4)+" rad/s.";
  if($("energyAlert")) $("energyAlert").textContent=alert;
  if($("energyFlow")) $("energyFlow").textContent=flow;
  if($("motionLevel")) $("motionLevel").textContent=motionLevel;
  if($("lossRate")) $("lossRate").textContent=pLoss.toFixed(4)+" W";
  if($("energyEta")) $("energyEta").textContent=eta;
}
...
function initPerformanceInfo(){
  ...
  el.textContent=`CPU logic: ${cores} luồng | GPU: ${gpu} | Canvas tăng tốc trình duyệt: bật | Mô phỏng thích ứng: ${$('performanceMode')?.checked?'bật':'tắt'}`;
}
...
function updateTargetLive(){
  ...
  if(!t){el.textContent='Chưa thiết lập mục tiêu';return;}
  ...
  el.textContent=`🎯 ${t.name} | khóa ${targetState.locked?'BẬT':'TẮT'} | X ${t.pos.x.toFixed(5)} m | Y ${t.pos.y.toFixed(5)} m | v ${speed.toFixed(5)} m/s | K ${t.K.toFixed(5)} J | cách chuột ${distMouse.toFixed(5)} m | phương vị ${bearing.toFixed(3)}° | TCA ${tau.toFixed(5)} s | gần nhất ${minRange.toFixed(5)} m`;
}
...
function updateHoverInfo(wPos,mx,my){
  ...
  if(dist<threshold){
    ...
    if($('hoverTarget')) $('hoverTarget').textContent=`👁️ ${nearest.name} | nhận diện liên tục | cách ${dist.toFixed(5)} m | tiến ${Math.max(0,-(dx*nearest.vel.x+dy*nearest.vel.y)/Math.max(1e-12,dist)).toFixed(5)} m/s | TCA ${tau.toFixed(5)} s`;
  }else if($('hoverTarget')) $('hoverTarget').textContent=`👁️ Rê chuột | gần nhất: ${nearest.name} | cách ${dist.toFixed(5)} m`;
  if($('hoverTitle')) $('hoverTitle').textContent=label;
  if($('hoverDetail')) $('hoverDetail').textContent=detail;
  if($('hoverCoords')) $('hoverCoords').textContent=`X ${wPos.x.toFixed(5)} m | Y ${wPos.y.toFixed(5)} m | khoảng cách ${dist.toFixed(5)} m`;
  if($('hoverAngles')) $('hoverAngles').textContent=`θ₁ ${(q[0]*180/Math.PI).toFixed(3)}° | θ₂ ${(q[1]*180/Math.PI).toFixed(3)}° | Δθ ${((q[1]-q[0])*180/Math.PI).toFixed(3)}°`;
  if($('hoverOmega')) $('hoverOmega').textContent=`ω₁ ${qd[0].toFixed(5)} rad/s | ω₂ ${qd[1].toFixed(5)} rad/s | α₁ ${qdd[0].toFixed(5)} rad/s² | α₂ ${qdd[1].toFixed(5)} rad/s²`;
  updateTargetLive();
}
...
function applySavedSettings(){let data={};try{data=JSON.parse(localStorage.getItem('atcx_settings')||'{}')}catch(e){}Object.keys(data).forEach(id=>{const el=$(id);if(!el)return;if(el.type==='checkbox')el.checked=!!data[id];else el.value=data[id];el.dispatchEvent(new Event(el.type==='checkbox'?'change':'input',{bubbles:true}))})}

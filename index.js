...
let predictionState={enabled:false,target:'m2',horizon:2,samples:6,lastUpdate:0};
...
// Bộ lọc trạng thái UI: tránh trạng thái nhảy liên tục khi giá trị vật lý
// dao động quanh ngưỡng. Trạng thái mới phải ổn định một khoảng thời gian
// trước khi được hiển thị.
let motionStatusState={
  current:null,
  candidate:null,
  candidateSince:0,
  minHold:0.75
...
function resetMotionStatusState(){
  motionStatusState.current=null;
  motionStatusState.candidate=null;
  motionStatusState.candidateSince=time;
}

function stabilizeMotionStatus(candidate){
  // Trạng thái nguy hiểm/dừng phải hiển thị ngay.
  if(candidate==="broken"){
    motionStatusState.current="broken";
    motionStatusState.candidate=null;
    return motionStatusState.current;
  }

  if(motionStatusState.current===null){
    motionStatusState.current=candidate;
    motionStatusState.candidate=null;
    return motionStatusState.current;
  }

  // Không đổi nếu vẫn cùng trạng thái.
  if(candidate===motionStatusState.current){
    motionStatusState.candidate=null;
    return motionStatusState.current;
  }

  // Bắt đầu/tiếp tục theo dõi trạng thái ứng viên.
  if(motionStatusState.candidate!==candidate){
    motionStatusState.candidate=candidate;
    motionStatusState.candidateSince=time;
    return motionStatusState.current;
  }

  // Chỉ chuyển trạng thái sau khi ứng viên ổn định đủ lâu.
  if(time-motionStatusState.candidateSince>=motionStatusState.minHold){
    motionStatusState.current=candidate;
    motionStatusState.candidate=null;
  }
...
function ui(){
  ...
  // Phân loại trước, sau đó lọc để tránh UI đổi trạng thái liên tục
  // khi K/ω nằm sát ranh giới giữa hai mức.
  let statusKey;
  if(brokenState){
    statusKey="broken";
  ...
  const stableStatus=stabilizeMotionStatus(statusKey);
...
function targetItemFromState(id,qq,ww,tt){
  const p=pointsFromState(qq,tt),v=velocitiesFromState(qq,ww,tt);
  const names={m1:'m₁',m2:'m₂',m1Mid:'Giữa thanh 1',m2Mid:'Giữa thanh 2',m1End:'Đầu thanh 1',m2End:'Đầu thanh 2'};
  const map={m1:['r1','v1'],m2:['r2','v2'],m1Mid:['c1','c1'],m2Mid:['c2','c2'],m1End:['r1','v1'],m2End:['r2','v2']};
  const k=map[id]||map.m2;
  return{id,name:names[id]||names.m2,pos:p[k[0]],vel:v[k[1]]};
}

function pointsFromState(qq,tt){
  const s=support(tt),e1=ep(qq[0]),e2=ep(qq[1]);
  return{
    s,
    r1:{x:s.x+S.L1*e1.x,y:s.y+S.L1*e1.y},
    r2:{x:s.x+S.L1*e1.x+S.L2*e2.x,y:s.y+S.L1*e1.y+S.L2*e2.y},
    c1:{x:s.x+.5*S.L1*e1.x,y:s.y+.5*S.L1*e1.y},
    c2:{x:s.x+S.L1*e1.x+.5*S.L2*e2.x,y:s.y+S.L1*e1.y+.5*S.L2*e2.y}
  };
}

function velocitiesFromState(qq,ww,tt){
  const sv=supportVel(tt),e1=dep(qq[0]),e2=dep(qq[1]);
  return{
    v1:{x:sv.x+S.L1*e1.x*ww[0],y:sv.y+S.L1*e1.y*ww[0]},
    v2:{x:sv.x+S.L1*e1.x*ww[0]+S.L2*e2.x*ww[1],y:sv.y+S.L1*e1.y*ww[0]+S.L2*e2.y*ww[1]},
    c1:{x:sv.x+.5*S.L1*e1.x*ww[0],y:sv.y+.5*S.L1*e1.y*ww[0]},
    c2:{x:sv.x+S.L1*e1.x*ww[0]+.5*S.L2*e2.x*ww[1],y:sv.y+S.L1*e1.y*ww[0]+.5*S.L2*e2.y*ww[1]}
  };
}

function predictionAcceleration(state,tt,forceRemaining){
  const saved=forceLeft;
  forceLeft=Math.max(0,forceRemaining);
  const a=accel(state.slice(0,2),state.slice(2),tt);
  forceLeft=saved;
  return a;
}

function predictionRK4(state,tt,dt,forceRemaining){
  const f=(z,t,fr)=>{const a=predictionAcceleration(z,t,fr);return[z[2],z[3],a[0],a[1]]};
  const k1=f(state,tt,forceRemaining);
  const k2=f(state.map((x,i)=>x+k1[i]*dt/2),tt+dt/2,Math.max(0,forceRemaining-dt/2));
  const k3=f(state.map((x,i)=>x+k2[i]*dt/2),tt+dt/2,Math.max(0,forceRemaining-dt/2));
  const k4=f(state.map((x,i)=>x+k3[i]*dt),tt+dt,Math.max(0,forceRemaining-dt));
  return state.map((x,i)=>x+dt*(k1[i]+2*k2[i]+2*k3[i]+k4[i])/6);
}

function predictTarget(id,horizon,samples){
  const out=[];
  let state=[q[0],q[1],qd[0],qd[1]];
  let tt=time;
  let fr=forceLeft;
  const count=Math.max(2,Math.min(12,Math.round(samples)));
  const end=Math.max(.05,Math.min(10,horizon));
  const dtMax=.0125;
  for(let i=1;i<=count;i++){
    const targetDt=end*i/count;
    let remain=targetDt-(tt-time);
    while(remain>1e-9){
      const h=Math.min(dtMax,remain);
      state=predictionRK4(state,tt,h,fr);
      tt+=h;
      fr=Math.max(0,fr-h);
      remain-=h;
    }
    const item=targetItemFromState(id,state.slice(0,2),state.slice(2),tt);
    out.push({t:tt-time,pos:item.pos,vel:item.vel});
  }
  return out;
}

function updatePrediction(force=false){
  if(!predictionState.enabled)return;
  const now=performance.now();
  if(!force&&now-predictionState.lastUpdate<33)return;
  predictionState.lastUpdate=now;
  const target=$('predictionTarget')?.value||targetState.selected||'m2';
  predictionState.target=target;
  const horizon=Math.max(.05,Math.min(10,Number($('predictionHorizon')?.value)||2));
  const samples=Math.max(2,Math.min(12,Math.round(Number($('predictionSamples')?.value)||6)));
  predictionState.horizon=horizon;
  predictionState.samples=samples;
  const items=targetItems();
  const current=items.find(x=>x.id===target)||items[1];
  if($('predictionCurrent'))$('predictionCurrent').textContent=uiText(`Tọa độ hiện tại: X ${current.pos.x.toFixed(5)} m | Y ${current.pos.y.toFixed(5)} m | Vx ${current.vel.x.toFixed(5)} m/s | Vy ${current.vel.y.toFixed(5)} m/s`);
  const rows=predictTarget(target,horizon,samples);
  const wrap=$('predictionTableWrap');
  if(!wrap)return;
  let html='<table class="prediction-table"><thead><tr><th>t +</th><th>X (m)</th><th>Y (m)</th><th>Vx (m/s)</th><th>Vy (m/s)</th></tr></thead><tbody>';
  rows.forEach(r=>{html+=`<tr><td>${r.t.toFixed(3)} s</td><td>${r.pos.x.toFixed(5)}</td><td>${r.pos.y.toFixed(5)}</td><td>${r.vel.x.toFixed(5)}</td><td>${r.vel.y.toFixed(5)}</td></tr>`});
  html+='</tbody></table>';
  wrap.innerHTML=uiText(html);
}
...
function initTargetControls(){
  const mode=$('targetMode'),sel=$('targetSelect'),lock=$('targetLock'),btn=$('targetSetup'),toggle=$('predictionToggle');
  if(!mode)return;
  const predictionToggle=()=>{
    predictionState.enabled=!!toggle?.checked;
    if($('predictionControls'))$('predictionControls').hidden=!predictionState.enabled;
    if(predictionState.enabled){
      if($('predictionTarget'))$('predictionTarget').value=targetState.selected;
      updatePrediction(true);
    }
  };
  mode.onchange=()=>{targetState.mode=mode.value;targetState.ready=false;updateTargetLive();};
  sel.onchange=()=>{targetState.selected=sel.value;targetState.ready=true;if($('predictionTarget'))$('predictionTarget').value=sel.value;updateTargetLive();updatePrediction(true);};
  lock.onchange=()=>{targetState.locked=lock.checked;updateTargetLive();};
  btn.onclick=setupTarget;
  if(toggle)toggle.onchange=predictionToggle;
  if($('predictionTarget'))$('predictionTarget').onchange=()=>{predictionState.target=$('predictionTarget').value;targetState.selected=predictionState.target;targetState.ready=true;if($('targetSelect'))$('targetSelect').value=predictionState.target;updateTargetLive();updatePrediction(true);};
  if($('predictionHorizon'))$('predictionHorizon').oninput=()=>updatePrediction(true);
  if($('predictionSamples'))$('predictionSamples').oninput=()=>updatePrediction(true);
  predictionToggle();
  updateTargetLive();
}
...
function applySavedSettings(){
  ...
  if(data.predictionEnabled!==undefined) predictionState.enabled=!!data.predictionEnabled;
  if(data.predictionTarget!==undefined) predictionState.target=data.predictionTarget;
  if(data.predictionHorizon!==undefined) predictionState.horizon=Number(data.predictionHorizon)||2;
  if(data.predictionSamples!==undefined) predictionState.samples=Number(data.predictionSamples)||6;
  ...
  if($('predictionToggle')) $('predictionToggle').checked=predictionState.enabled;
  if($('predictionTarget')) $('predictionTarget').value=predictionState.target;
  if($('predictionHorizon')) $('predictionHorizon').value=predictionState.horizon;
  if($('predictionSamples')) $('predictionSamples').value=predictionState.samples;
}

function initSettingsButton(){const b=$('openSettings');if(b)b.onclick=()=>{location.href='settings.html'}}
applySavedSettings();initDraggablePanel("targetPanel","atcx_target_panel");initDraggablePanel("predictionControls","atcx_prediction_panel");initDraggablePanel("hoverInfo","atcx_hover_panel");initSettingsButton();
